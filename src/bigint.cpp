...
namespace bigint {
	...
	unsigned long UnsignedBigint::numberOfZeros() const {
		unsigned int i;
		unsigned long acc = 0;
		for (i = 0; i < number.size(); i++) {
			if(number.at(i) == 0) {
				acc += sizeof(UBIGGEST_TYPE)*8;
			}
		}
		UBIGGEST_TYPE res;
		for (unsigned int j = 0; i < sizeof(UBIGGEST_TYPE)*8 && res!=0; i++) {
			UBIGGEST_TYPE mask = ipow(2, j);
			res = number.at(i) & mask;
			if(res == 0) {
				acc++;
			}
		}
